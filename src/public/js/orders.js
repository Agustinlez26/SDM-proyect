...
const openOrderDetails = async order => {
    ...
    try {
        ...
        list.innerHTML = details.length
            ? details.map(item => `<tr><td data-label="SKU"><span class="order-sku">${esc(item.sku || 'S/C')}</span></td><td data-label="Producto"><strong>${esc(item.name)}</strong></td><td data-label="Sucursal">${esc(item.branch_name)}</td><td data-label="Cantidad" class="text-right"><span class="order-quantity-badge">${esc(item.quantity)}</span></td></tr>`).join('')
...
const render = () => {
    ...
    $('orders-list').innerHTML = state.orders.map(o => `<tr><td data-label="Pedido"><strong>${esc(o.order_number)}</strong></td><td data-label="Canal">${esc(channelLabel(o.channel))}</td><td data-label="Cliente / referencia">${esc(o.customer_reference)}</td><td data-label="Sucursal">${esc(o.branch_name)}</td><td data-label="Productos"><button class="order-details-button" data-details="${o.id}" type="button" title="Ver productos del pedido"><span class="material-symbols-outlined">visibility</span> Ver${Number(o.item_count) ? ` (${o.item_count})` : ''}</button></td><td data-label="Estado"><span class="order-status ${o.status}">${esc(statusLabel(o.status))}</span></td><td data-label="Acciones" class="order-actions-cell"><div class="order-actions">${o.status === 'reserved' ? `${canConfirm ? `<button data-complete="${o.id}">Confirmar</button>` : ''}<button class="danger" data-cancel="${o.id}">Cancelar</button>` : '<span class="order-no-actions">Sin acciones pendientes</span>'}</div></td></tr>`).join('') || '<tr class="orders-empty-row"><td colspan="7">Todavía no hay pedidos en este canal.</td></tr>'
